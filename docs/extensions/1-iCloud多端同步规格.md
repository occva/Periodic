...
- “使用云端替换此 Mac”必须先生成可恢复的本地备份，不能直接删除原库。
...
1. 抽取数据集描述、同步元数据和纯合并器；完成双本地库模拟测试。
2. 接入 CloudKit development container，仅同步订阅和历史，不开放 UI。
3. 完成首次迁移、状态页、冲突页和 tombstone。
4. 接入付款、模板、分类和图片资产。
5. 完成账号切换、备份联动、压力测试和 production schema 部署。
