...
每行只有一个事件。标签包含图标、名称、剩余天数和到期状态；原“正常”状态改为“N 天内”，其他状态文案保持不变。起止日期齐全时附剩余比例。标签的开始位置跟随日期锚点，文字可以向右延伸，但不能为了放下文字移动日期锚点。停用行视觉弱化，保留真实日期和状态。
...
先枚举视口及少量预取范围中的真实刻度日期，再映射坐标，不通过重复累加固定像素宽生成月份。只渲染可见刻度，行网格复用背景绘制，不每行各建一份完整网格视图树。今天圆点独立使用日号，即使在 3/5 年视图中也不把月份数字误作当天日期。

事件锚点在 `0…W` 时显示点和向右标签；标签宽度受剩余空间限制并可截断，完整内容通过悬停、详情及 VoiceOver 获取。锚点越过左/右边界时改为边缘方向按钮；按钮用于定位，必须与真正事件点的视觉样式区分，不暗示到期日就在边缘。

### 4.3 滚动同步与虚拟化

时间轴不维护与日期中心分离的内容偏移。当前范围直接映射到内容区完整宽度，窗口缩放时保持中心日期和跨度不变并重新计算 `pixelsPerDay`；刻度文字根据可用间距抽样，网格与事件仍使用真实日期坐标。触控板横向滚动按当前 `pixelsPerDay` 换算为中心日期变化，顶部刻度、网格与事件使用同一个中心同步重建；纵向滚动事件继续交给订阅行列表。前一范围、今天、后一范围仍按完整跨度改变中心日期，不引入另一套水平滚动真值。
