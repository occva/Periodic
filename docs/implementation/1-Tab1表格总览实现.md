...
| 6 | 到期状态 | 日期未知、已过期、今天到期、7 天内、30 天内、N 天内、永久有效 | 仅用精确剩余天数替代原“正常”，文字和状态图形配合颜色 |
...
切换为终生时在草稿内清空 cycleMonths/expiry、关闭提醒，并展示影响提示；保存确认才改库，取消保留原记录。保存后若没有终生周期记录则自动补一条至 `2099-12-31`。切回 recurring 草稿默认月度、expiry=nil、提醒开启，金额数值保留但标签改为周期价格，要求用户核对。仅合法保存改变类型，历史付款金额及覆盖日期不随类型变化；终生首次付款 periodEnd=nil。
...
入口统一为“新建 → 从模板”、空库添加服务、表格底部添加服务。选择器只承担模板选择：单击模板卡片立即关闭选择器并打开已预填的共享新建表单，不设置勾选态或“下一步”按钮；取消不产生草稿写入。模板管理页继续承担分类和模板维护。两种场景均只显示一个模板搜索框。
...
内置模板分类沿用固定七类，但按服务的主要用途归类：通讯服务包含运营商、网络、即时消息、社交网络与约会社区；家庭日常包含零售、出行、智能家居、健康、冥想与健身；工具产品包含安全、存储、效率及金融分析工具；影音娱乐包含视频、音乐、游戏及创作者内容会员；餐饮零食包含外卖、餐饮和食材配送。“其他”只作为确实跨领域或无法可靠判断时的兜底，不用来承载已有明确用途的社交或健康服务。

品牌图标使用显式用户操作：编辑器同时提供“选择本地图片”和“从 Apple 搜索”。本地入口通过系统文件选择器授权，只接受可解码且不超过 10 MB 的 PNG/JPEG，复制到应用沙盒后再引用。只有用户提交 Apple 搜索关键词时，才向 Apple Search API 发送关键词与所选地区；Return 与“搜索”按钮执行同一动作，Escape 与“取消”按钮关闭选择界面。选择结果缓存到应用沙盒；离线或缓存不可用时保留图标不可用状态，绝不冒充为另一个 SF Symbol。只有模板或订阅从未设置图标时，才按名称稳定选择一个 SF Symbol 占位。应用不自动为内置模板发起网络请求，也不把查询发送给第三方服务。

模板卡片可拖到侧边栏的内置分类或自定义分类。用户模板直接更新持久化分类；内置模板保留离线目录原始内容，另存本地分类覆盖，不生成用户副本。自定义分类被删除时，两种来源的关联模板都回到“其他”。
...
| tableRemainingProgress | 表格专用；recurring 有到期日时 clamp(D / 100, 0, 1)，不依赖开始日；终生/缺到期日 nil |
