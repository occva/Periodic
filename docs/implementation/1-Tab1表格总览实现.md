...
| TECH、NAV、UI、QA | 应用装配、多窗口、持久化、可访问性 | AppServices、WindowSession、StoreActor | 原生、离线、无产品网络请求 |

### 1.1 页面归属

- 主窗口使用 `WindowGroup`，以 `NavigationSplitView` 侧边栏呈现三个逻辑 Tab：`overview` 表格总览、`timeline` 时间线日历、`dashboard` 统计仪表盘。
- Tab1 是默认入口；详情、新建、编辑、续费、付款维护均为共享 Sheet 内容，Tab2、Tab3 调用同一套组件。
...
| 8 | 周期/一次性金额与币种 | 如 `CNY 88.00`；终生价格标明一次性 | 价格不是付款记录 |
...
| 表单分区 | 字段与默认值 | 校验及保存含义 |
| --- | --- | --- |
| 基本信息 | 名称空、类型其他、状态订阅中、后备 SF Symbol | 名称 trim 后非空；同名允许 |
...
| 图标 | SF Symbol 或 PNG/JPEG | 解码成功后预览；确认时复制入沙盒 |
...
手动新增/编辑只保存历史，不改主表格/主日历的当前周期及提醒。点击“应用到当前设置”另行预览：复制开始/结束及类型，预设周期带入对应 M，custom 需用户选择 1/3/6/12；lifetime 清周期/到期并关闭提醒。默认保留当前价格、币种和管理状态，可明确选择采用已知本期价格或恢复 active。周期型应用保留原提醒意图；从 lifetime 变 recurring 时仍默认保持 false，用户可主动开启。
...
入口统一为“新建 → 从模板”、空库添加服务、表格底部添加服务；根 Sheet 内从模板选择转到共享表单，返回保留模板查询。首屏包含独立搜索、全部/按类型布局、来源筛选（全部/内置/我的模板）和空白新建。
...
内置目录随应用离线提供，名称可覆盖截图所示的影音、工具、生活与通讯服务，例如 Apple Music、Netflix、ChatGPT、Notion、Office 365、京东 Plus 等；不从截图假定现价、有效期或购买状态。内置内容只读，“复制为我的模板”后才能修改；首版不做在线市场、自动抓取和模板执行脚本。
